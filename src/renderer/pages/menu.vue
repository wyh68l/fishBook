    ...
    <div class="body">
      ...
      <div class="main-item book-list" v-if="currentIndex === 0">
        ...
        <div class="book-item" :class="{active: book && book.title === item.title}" v-for="item in books" :key="item.title">
          ...
          <div class="item-th">
            <input class="input" type="text" :value="((300 * (item.page -1) / item.size) * 100).toFixed(2)" @change="(e)=>{return setSpeed(e,item)}"/>%
          </div>
          <div class="item-th">
    ...
    methods: {
      checkMenu(index) {
        this.currentIndex = index
	      console.log(this.$store.state.Book.books)
      },
      // 选择文件
      selectFile() {
      ...
      async afterFileSelected(e) {
	    ...
	    setSpeed(e,item){
        let page = parseInt((((e.target.value) * item.size)/30000)+1)
